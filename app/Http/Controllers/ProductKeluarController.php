...
class ProductKeluarController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = Product_Keluar::with(['product', 'customer'])->select('product_keluar.*');
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($id) {
                // ---- UPDATE logic ----
                $pk = Product_Keluar::findOrFail($id);
                ...
                $pk->update($request->only(['product_id','customer_id','qty','tanggal']));
            } else {
                // ---- CREATE logic ----
                ...
                Product_Keluar::create($request->only(['product_id','customer_id','qty','tanggal']));
    ...
    {
        $row = Product_Keluar::findOrFail($id);
...
{
    $invoices = Product_Keluar::with(['product', 'customer'])->latest();
