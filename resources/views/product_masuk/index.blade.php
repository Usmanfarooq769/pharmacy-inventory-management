...
$(function() {
    ...
    // SweetAlert helpers
    const swalSuccess = (msg) => Swal.fire({ icon: 'success', title: 'Success', text: msg, confirmButtonText: 'OK' });
    const swalError   = (msg) => Swal.fire({ icon: 'error', title: 'Error', text: msg, confirmButtonText: 'OK' });

    // DataTable
    var table1 = $('#products-in').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('product_masuk.data') }}",
        dom: 'Blfrtip',
    ...
    processing: true,
    serverSide: true,
    ajax: "{{ route('product_masuk.invoiceData') }}", 
     dom: 'Blfrtip',
        buttons: ['copy','csv','excel','pdf','print'],
    columns: [
        { data: 'id', name: 'id' },
        ...
        $.ajax({
            ...
            error: function(xhr) {
                if (xhr.status === 422) {
                    ...
                    var arr = [];
                    $.each(errors, function(k, v) { arr.push(v[0]); });
                    swalError(arr.join('\n'));
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    if (xhr.responseJSON && xhr.responseJSON.message) swalError(xhr.responseJSON.message);
